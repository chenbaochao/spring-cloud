		...
		$.ajax({
			...
			success : function(result) {
				...
				if (result.status == 'SUCCESS') {
                    $(".part4").children("h3").text("恭喜" +$('#adminNo').val() + "，您已注册成功，现在开始您的购物之旅吧！");
                    $(".part1").hide();
                    $(".part4").show();
					...
					$(".message").show();
					$(".message").children("label").text(result.errMsg);
				}
			}
		})
	});
...
$(function() {
	$('#kaptchaImage').click(function() {
		$(this).attr('src',  '/user/captcha?rand=' + Math.floor(Math.random() * 100));
	})
});
